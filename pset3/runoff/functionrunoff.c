...
void tabulate(void)
{
    ...
    for (int i = 0; i < voter_count; i++)
    {
        ...
        {
            int k = 0;
            if (candidates[j].eliminated == true) //voir si cette ligne ignore bien plusieurs candidats éliminés.
            {
                k++;
            }
            else if (candidates[j].name == candidates[preferences[i][k]].name)
            {
                candidates[j].votes++;
            }
        }
...
bool print_winner(void)
{
    ...
    printf("Le nombre de vote pour gagner neccessaire est: %i\n", vote_needed);
